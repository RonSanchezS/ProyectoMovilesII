...
import { Alien, Personaje } from 'src/app/models/Personaje';
import { HttpClient, HttpClientModule } from '@angular/common/http';
import { Heroe } from 'src/app/models/Heroe';
import { Viloes } from 'src/app/models/Villano';
import { Antiheroe } from 'src/app/models/Antiheroes';
import { AlienNuevo } from 'src/app/models/newAlien';
import { Humanos } from 'src/app/models/Humano';
...
export class InicioPage implements OnInit {
  ...
  heroes: Heroe[] = [this.hombreAraña, this.ironMan, this.panteraNegra];
  ...
  doctorDoom: Viloes = {
    ...
    movies: ['./movies/fantastic-four-1.jpg', './movies/fantastic-four-2.jpg'],
  };
  duendeVerde: Viloes = {
    name: 'Duende Verde',
  ...
  viloes: Viloes[] = [this.doctorDoom, this.redskull, this.duendeVerde];
  ...
  antiheroes: Antiheroe[] = [this.deadpool, this.venom, this.justiciero];
  ...
  aliens: AlienNuevo[] = [this.ronan, this.thanos, this.talos];
  ...
  humanos: Humanos[] = [this.happy, this.maryJane, this.howard];
  constructor(
  ...
  filtrar(tipo: string) {
    const divHeroes =
      this.elementref.nativeElement.querySelector('.itemHeroes');
    const divVillanos =
      this.elementref.nativeElement.querySelector('.itemVillanos');
    const divHumanos =
      this.elementref.nativeElement.querySelector('.itemHumanos');
    const divAliens =
      this.elementref.nativeElement.querySelector('.itemAliens');
    const divAntiHeroes =
      this.elementref.nativeElement.querySelector('.itemAntiheroe');
    switch (tipo) {
      case 'heroe':
  ...
  buscar() {
    //for each heroe in heroes
    this.heroes.forEach((element) => {
      this.personajes.push(element);
    });
    this.viloes.forEach((element) => {
      this.personajes.push(element);
    });
    this.humanos.forEach((element) => {
      this.personajes.push(element);
    });
    this.aliens.forEach((element) => {
      this.personajes.push(element);
    });
    this.antiheroes.forEach((element) => {
      this.personajes.push(element);
    });
  }
}
